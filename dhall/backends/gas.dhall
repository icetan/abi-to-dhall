...
let funIndexedArgToDhallFunValue
    : SimpleIArg → Text
    =   λ(iarg : SimpleIArg)
      → "${iarg.value.type}(\${arg${Natural/show iarg.index}.${iarg.value.type}})"
...
let toOutput : Text → Text = λ(val : Text) → "// OUTPUT \\\"\${id}=\${${val}}\\\""
...
let sendValue =
      ...
      → "\"\${name}(\${address.address}).${fun.name}(${funArgsToDhallFunValue fun.inputs});\""
...
let callDef =
      ...
      → funArgNamesToDhallConcatDefs
          ([ "address" ] # funArgsToDhallArgNames fun.inputs)
          [ "[ { mapKey = tag, mapValue = \"${funArgsToDhallCall fun.outputs} \" ++ (List/fold DefEntry (backend.defineMem tag \"\${address.address}.${fun.name}(${funArgsToDhallFunValue fun.inputs})\") Text (λ(t : DefEntry) → λ(_ : Text) → t.mapValue) \"\") } ]"
          ]
...
let createDef =
      ...
      → funArgNamesToDhallConcatDefs
          ...
          [ "[ { mapKey = tag, mapValue = \"\${name} \" ++ (List/fold DefEntry (backend.defineMem tag \"new \${name}(${funArgsToDhallFunValue constructor.inputs})\") Text (λ(t : DefEntry) → λ(_ : Text) → t.mapValue) \"\") } ]"
          ]
